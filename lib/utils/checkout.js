  ...
  return {
    lines: items.map((item) => {
      ...
      if (item?.purchaseType === 'recurring') {
        line.sellingPlanId =
          get(item, 'storefrontData.sellingPlanGroups.edges[0].node.sellingPlans.edges[0].node.id') ??
          get(item, 'storefrontData.variants.edges[0].node.sellingPlanAllocations.edges[0].node.sellingPlan.id') ??
          '';
      }
